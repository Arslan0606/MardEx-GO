package uz.star.mardex.utils.custom
...
class MoneyEditText : androidx.appcompat.widget.AppCompatEditText {
    ...
    private fun initView(context: Context, attrs: AttributeSet?) {
        ...
        if (attrs != null) {
            ...
            val attrArray = context.obtainStyledAttributes(attrs, R.styleable.EasyMoneyWidgets, 0, 0)
            try {
                ...
                _showCurrency = attrArray.getBoolean(R.styleable.EasyMoneyWidgets_show_currency, false)
                    ...
                    if (valStr == "") {
                        ...
                        if (valStr.contains(".")) {
                            if (valStr.indexOf(".") == valStr.length - 1) {
                                ...
                                val front = getDecoratedStringFromNumber(valStr.substring(0, valStr.length - 1).toLong())
                                setText("$front.")
                            } else {
    ...
    private fun getDecoratedStringFromNumber(number: Long): String {
        ...
        if (_showCommas && !_showCurrency) formatter.applyPattern(numberPattern) else if (_showCommas && _showCurrency) formatter.applyPattern("$_currencySymbol $numberPattern") else if (!_showCommas && _showCurrency) formatter.applyPattern(
            "$_currencySymbol "
